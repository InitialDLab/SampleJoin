...
mk_order(DSS_HUGE index, order_t * o, long upd_num)
{
	...
	/* change the distribution of number of lineitems per order from 
	 * unif to zipf */
	/* RANDOM(o->lines, O_LCNT_MIN, O_LCNT_MAX, O_LCNT_SD); */
	o->lines = O_LCNT_MIN + sample_from_zipf(&lineitem_zipf, O_LCNT_SD);

	for (lcnt = 0; lcnt < o->lines; lcnt++)
	{
		o->l[lcnt].okey = o->okey;;
		o->l[lcnt].lcnt = lcnt + 1;
		RANDOM64(o->l[lcnt].quantity, L_QTY_MIN, L_QTY_MAX, L_QTY_SD);
