...
void init_zipf(zipf_distribution *dist, DSS_HUGE N, double p) {
	int i;

	dist->N = N;
	dist->p = p;
	dist->unnormalized_cdf = (double *) malloc(sizeof(double) * N);
	
	dist->unnormalized_cdf[0] = 1.0;
	for (i = 1; i < N; ++i) {
		dist->unnormalized_cdf[i] = dist->unnormalized_cdf[i-1] + 1.0 / pow(i+1, p);
	}
}

DSS_HUGE sample_from_zipf(zipf_distribution *dist, int stream) {
	DSS_HUGE i;
	double v = rand_real(stream) * dist->unnormalized_cdf[dist->N-1];

	for (i = 0; v > dist->unnormalized_cdf[i]; ++i);
