...
usage (void)
{
	...
	fprintf (stderr, "-z <p> -- generate the number of lineitems per order ~ O_LCNT_MIN + zipf(O_LCNT_MAX - O_LCNT_MIN + 1, p) (default: p = 0.0)\n");
	fprintf (stderr,
...
main (int ac, char **av)
{
	...
	init_zipf(&lineitem_zipf, O_LCNT_MAX - O_LCNT_MIN + 1, lineitem_zipf_p);
#if (defined(WIN32)&&!defined(_POSIX_))
	for (i = 0; i < ac; i++)
	{
