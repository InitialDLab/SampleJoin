...
static global_settings query5Settings;
    ...
    for (auto f5_enu = filters.at(4)->getEnumerator(); f5_enu->Step();) {
        ...
        for (auto matching_indexes : Table4_hash[value]) {
            ...
            output_file << count << ' '
                ...
                << std::to_string(Table_4->get_int64(std::get<3>(matching_indexes), 1)) << ' '
                << std::to_string(Table_5->get_int64(f5_enu->getValue(), 1)) << '\n';
            count++;
        }
    }
...
int64_t randomjefastJoin_range(std::ofstream &output_file, int64_t output_count, std::shared_ptr<jefastIndexLinear> jefast, int64_t min, int64_t max) {
    ...
    std::uniform_int_distribution<weight_t> distribution(min, max - 1);
    ...
    for (int64_t tuples_remaining = output_count; tuples_remaining > 0; tuples_remaining -= random_values.size())
    {
        ...
        for (int i = 0; i < random_values.size(); ++i) {
            ...
            output_file << i << " "
                ...
                << std::to_string(Table_5->get_int64(results.at(4), 1)) << '\n';
        }
    }
...
void joinAttribExp(std::shared_ptr<jefastIndexLinear> jefast_base)
{
    ...
    for (auto min_orderkey : { 5000000, 4000000, 3000000, 2000000, 1000000 })
    {
        ...
        if(query5Settings.joinAttribHash)
        {
            timer.reset();
            timer.start();
            ...
            data_map->appendArray(name + "_cardinality", long(count));
        }

        if(query5Settings.joinAttribJefast)
        {
            timer.reset();
            timer.start();
            ...
            jefast_index_wfilter_builder.AddFilter(min_orderkey, std::numeric_limits<jfkey_t>::max(), 3);
            auto jefast_index_wfilter = jefast_index_wfilter_builder.Build();
            timer.stop();
            std::cout << "jefast took " << timer.getMilliseconds() << " milliseconds to build for min=" << min_orderkey << " cardinality=" << jefast_index_wfilter->GetTotal() << std::endl;
...
void nonJoinAttribTrial(int id, float min_accbal, int64_t min_date, int64_t max_date, int64_t max_region, std::shared_ptr<jefastIndexLinear> jefast_base)
{
    ...
    if(query5Settings.nonJoinAttribHash)
    {
        timer.reset();
        timer.start();
        auto count = exactJoinNoIndex("query5_full.txt", filters);
    ...
    if(query5Settings.nonJoinAttribJefast)
    {
        timer.reset();
        timer.start();
        //auto jefast_index_wfilter_builder = jefastBuilderWJoinAttribSelection(jefast_base);
...
int main(int argc, char** argv) {
    query5Settings = parse_args(argc, argv);

    Timer timer;
    setup();

    if(query5Settings.hashJoin)
    {
        // do full hash join
        std::vector< std::shared_ptr<jefastFilter> > filters(5);
        filters.at(0) = std::shared_ptr<jefastFilter>(new all_jefastFilter(nation_table, Table_Nation::N_NATIONKEY));
    ...
    if(query5Settings.buildJefast)
    {
        ...
        data_map->appendArray("jefast_building", long(timer.getMilliseconds()));
        // condition c_acctbal > '[min balance]'
    }

    if(query5Settings.jefastSample)
    {
       //jefast_index->print_search_weights();
    ...
    if (query5Settings.jefastModify)
    {
        // do random modifications for supplier, table 2
        int table_id = 2;

        data_map->appendArray("modifyCount", query5Settings.modifyCount);

        // generate random ids to remove/insert
        std::vector<jfkey_t> modify_records = generate_unique_random_numbers(query5Settings.modifyCount, 0, customer_table->row_count());
        // do delete
        timer.reset();
        timer.start();
        for (auto record_id : modify_records) {
            //std::cout << "removing " << record_id << std::endl;
            jefast_index->Delete(table_id, record_id);
        }
        timer.stop();
        data_map->appendArray("DeleteTime", long(timer.getMilliseconds()));

        std::cout << "delete of " << query5Settings.modifyCount << " took " << timer.getMilliseconds() << "ms. The cardinality is now " << jefast_index->GetTotal() << std::endl;

        // do insert
        timer.reset();
        timer.start();
        for (auto record_id : modify_records) {
            jefast_index->Insert(table_id, record_id);
        }
        timer.stop();

        std::cout << "insert of " << query5Settings.modifyCount << " took " << timer.getMilliseconds() << "ms. The cardinality is now " << jefast_index->GetTotal() << std::endl;
    }

    if(query5Settings.olkenSample)
    {
       // do 100000% using olkin join
       ...
       data_map->appendArray("baseline_100000pRejected", long(rejected));
    }

    if (query5Settings.jefastRate)
    {
    ...
    if(query5Settings.threading)
    {
        ...
        for (int64_t i = 1; i <= max_threads; i *= 2) {
            ...
            // start the threads
            for (int t = 0; t < i; ++t) {
                results.emplace_back(std::async(randomjefastJoinT, query5Settings.null_file_name, requests / i, jefast_index));
            }
    ...
    if(query5Settings.joinAttribHash || query5Settings.joinAttribJefast)
    {
        // do experiment with join attribute with selection condition.
        joinAttribExp(jefast_index);
    }

    if (query5Settings.nonJoinAttribHash || query5Settings.nonJoinAttribJefast)
    {
        doNonJoinAttribExp(jefast_index);
    }
