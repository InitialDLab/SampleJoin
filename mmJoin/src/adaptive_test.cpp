...
int main(int argc, char** argv) {
    ...
    std::shared_ptr<TableGeneric> table1(new TableGeneric(settings.TableA_filename, settings.delimiter, -1, settings.AR));
    std::shared_ptr<TableGeneric> table2(new TableGeneric(settings.TableB_filename, settings.delimiter, settings.BL, settings.BR));
    std::shared_ptr<TableGeneric> table3(new TableGeneric(settings.TableC_filename, settings.delimiter, settings.CL, -1));
    ...
    for (int i = 0; i < 50000000; ++i)
    {
        if (i%batch_size == 0)
        {
            ...
            std::cout << "^ time " << std::chrono::duration_cast<std::chrono::duration<double>>(batch_t2 - batch_t1).count() << std::endl;
